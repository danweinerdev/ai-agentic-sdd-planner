...
├── Diagrams/                     # Standalone diagrams (flat — generated by /sdd-planner:diagram)
│   └── <slug>.md
...
- `/sdd-planner:implement` sets `status: complete` when the final phase finishes; `/sdd-planner:debrief` and `/sdd-planner:tend` backfill it if missed
...
| `/sdd-planner:diagram` | Generate Mermaid diagrams from artifacts → `Diagrams/<slug>.md` |
...
```
Use `/sdd-planner:poke-holes` before approving any artifact. Use `/sdd-planner:tend` periodically for hygiene. Use `/sdd-planner:decide` to record or look up decided truths at any point.
